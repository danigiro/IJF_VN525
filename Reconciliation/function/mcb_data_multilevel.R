...
mcb_data <- function(DF){
  if(!("FoReco" %in% colnames(DF))){
    ...
    for(j in 1:max(DF$Replication)){
      ...
      if(NROW(recf_list[[j]])<12){
        recf_list[[j]] <- rbind(recf_list[[j]], matrix(0, nrow = 12-NROW(recf_list[[j]]), ncol = NCOL(recf_list[[j]])))
      }
