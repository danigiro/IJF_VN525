...
nn_data$time <-  NA
save.image(file = "./Reconciliation/ETSlev/cslcc_555_mLCCendo_preosqp.RData")
problem <- c()
for(row_nn in 1:NROW(nn_data)){
  ...
  nameLCC <- paste0("mLCC", sapply(names(obj$levrecf), 
                                   function(x) strsplit(x, "_")[[1]][3]), "endo")
...
save(DFcslev, time_cslev, nn_data_cslev, 
     file="./Reconciliation/ETSlev/VN555_cslccd_mLCCendo.RData")