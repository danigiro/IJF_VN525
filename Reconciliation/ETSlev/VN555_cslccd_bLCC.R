...
#' Output files: VN555_cslccd_bLCC.RData
#'
...
time_cslev <- array(NA, dim = c(test_length, length(K_vec), 1),
                    dimnames = list(NULL, K_vec, c("bCCCexod")))
...
nn_data$time <-  NA
save.image(file = "./Reconciliation/ETSlev/cslcc_555_bLCC_preosqp.RData")
problem <- c()
...
save(DFcslev, time_cslev, nn_data_cslev, 
     file="./Reconciliation/ETSlev/VN555_cslccd_bLCC.RData")
