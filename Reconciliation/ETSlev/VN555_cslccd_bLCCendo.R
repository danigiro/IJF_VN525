...
#'       - bLCCendo (basef + endogenous + diagonal cov)
#'
#' Input files: DFbase.RData, Residuals.RData, VN525.RData, VN525_features.RData
#' Output files: VN555_cslccd_bLCCendo.RData
#'
...
time_cslev <- array(NA, dim = c(test_length, length(K_vec), 1),
                    dimnames = list(NULL, K_vec, c("bCCCendod")))
...
nn_data_all <- DF %>% filter(nn=="FALSE")%>%
  select(`R-comb`,K, Replication) %>% unique()

nn_data <- nn_data_all %>%
  select(K, Replication) %>% unique()

nn_data$time <-  NA
save.image(file = "./Reconciliation/ETSlev/cslcc_555_bLCCendo_preosqp.RData")
problem <- c()
for(row_nn in 1:NROW(nn_data)){
  ...
  ty <- nn_data_all %>% filter(K == K_vec[i], Replication ==j) %>% pull(`R-comb`)
  ...
  nameLCC <- paste0("bLCC", sapply(names(obj$levrecf), 
                                   function(x) strsplit(x, "_")[[1]][3]), "endo")
  
  for(lev in which(nameLCC %in% ty)){
    Df1 <- cbind(Fltr, "Forecasts" = as.vector(obj$levrecf[[lev]]),
                 "R-method" = "cslcc", "R-comb" = nameLCC[lev], 
                 nn = all(obj$levrecf[[lev]]>=0), 
                 FoReco = "osqp")
    Df1 <- Df1[c(names(DFbase), "nn", "FoReco")]
    DF <- rbind(DF, Df1)
  }
  ...
  cat("\n  ", formatC(i, width=2, flag="0"), "  |  ", 
      ...
      formatC(length(ty), width=5), " |  ", row_nn, "/", NROW(nn_data), sep="")
}
nn_data_cslev <- nn_data
DFcslev <- DF
save(DFcslev, time_cslev, nn_data_cslev, 
     file="./Reconciliation/ETSlev/VN555_cslccd_bLCCendo.RData")