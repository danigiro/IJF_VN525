...
for (j in 1:test_length) { #test_length
  for(i in 1:length(K_vec)){
    ...
    Start <- Sys.time()
    objH <- suppressMessages(lccrec(basef = basef, C = C, nl = nl, const = "endo",
...
nn_data$time <-  NA
save.image(file = "./Reconciliation/ETSlev/cslcc_555_bCCCred_endo_preosqp.RData")
problem <- c()
for(row_nn in 1:NROW(nn_data)){
  ...
  bCCCred <- Reduce("+", obj$levrecf[-length(obj$levrecf)])/(length(obj$levrecf)-1)
  Df1 <- cbind(Fltr, "Forecasts" = as.vector(bCCCred),
               "R-method" = "cslcc", "R-comb" = "bCCCred_endo", 
               nn = all(bCCCred>=0), 
               FoReco = "osqp")
...
save(DFcslcc, time_cslev, nn_data_cslev, 
     file="./Reconciliation/ETSlev/VN555_cslccd_bCCCred_endo.RData")