...
for(j in 1:test_length){
  ...
  Df1 <- cbind(Fltr, "Forecasts" = as.vector(t(Recon_PointF)),
               "R-method" = "cs", "R-comb" = "mix-CCC_endo", 
               nn = nn_val, 
               FoReco = "direct")
  Df1 <- Df1[names(DFb)]
  DF <- rbind(DF, Df1)
...
DFmix <- DF
save(DFmix, 
     file="./Reconciliation/ETSlev/VN555_CCCmix_endo.RData")